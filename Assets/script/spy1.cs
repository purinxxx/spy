...
public class spy1 : MonoBehaviour
{
    ...
    void Update()
    {
        if (manager.spy1tern)
        {
            ...
            if (manager.item1)
			{
				...
				manager.message.text = "スパイ１はプロテクターを使った";
				manager.logcontent.text = manager.logcontent.text + "\nスパイ１はプロテクター使った\n";
                //manager.saikorobutton.SetActive(false);
                manager.saikorobutton.GetComponent<Button>().interactable = false;
                //manager.itembutton.SetActive(false);
            ...
            if (manager.item2)
            {
                manager.message.text = "スパイ１は車を使った";
                manager.logcontent.text = manager.logcontent.text + "\nスパイ１は車使った\n";
                bairitu = 2;
                manager.item2 = false;
            }
            if (manager.item3)
            {
                manager.message.text = "スパイ１はヘリを使った";
                manager.logcontent.text = manager.logcontent.text + "\nスパイ１はヘリを使った\n";
                bairitu = 3;
                manager.item3 = false;
            }
            ...
            if (manager.susumu)
			{
                ...
                for (int i = 1; i <= Mathf.Abs(me); ++i)
                {
                    ...
                    camerapos.y = pos.y;
                    camerapos.z = -10;
					StartCoroutine(idou(pos, camerapos, i, manager.playerpos[1]));
                    if (k == manager.bom2pos[0])
                    {
                        manager.spylife[0] -= 1;
	...
	private IEnumerator idou(Vector3 player, Vector3 camera, int time, int spy1pos)
    {
        time = Mathf.Abs(time);
